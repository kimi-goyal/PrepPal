  ...
  return (
    <div className="space-y-10 font-sans">
          ...
          <div key={idx} className="p-5 rounded-2xl bg-white border border-[#E3E1DB]">
            <div className="flex items-center justify-between mb-4">
              <div className="w-9 h-9 bg-[#DCEAE6] rounded-lg flex items-center justify-center">
                <item.icon size={16} className="text-[#1E5F53]" />
              </div>
              <p className="text-[#6E6A62] text-xs uppercase tracking-wide text-right">{item.title}</p>
            </div>
            <h3 className="text-2xl font-mono font-medium text-[#171A21] mb-3">
              {item.value}
            </h3>
            <div className="w-full h-1 bg-[#E3E1DB] rounded-full overflow-hidden">
              <div
                className="h-full bg-[#1E5F53] rounded-full transition-all duration-700"
                style={{ width: `${item.barWidth}%` }}
              ></div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-7 rounded-2xl border border-[#E3E1DB]">
          <div className="mb-6">
            <h4 className="text-lg font-display font-medium text-[#171A21]">Confidence trend</h4>
            <p className="text-[#6E6A62] text-sm">Performance across all sessions</p>
          </div>
          ...
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={charts.confidenceTrend}>
              ...
              <Line
                ...
                activeDot={{ r: 6, stroke: "#1E5F53", strokeWidth: 2, fill: "#fff" }}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-white p-7 rounded-2xl border border-[#E3E1DB] h-full">
          <div className="mb-6">
            <h4 className="text-lg font-display font-medium text-[#171A21]">Tone analysis</h4>
            <p className="text-[#6E6A62] text-sm">Communication style breakdown</p>
          </div>
        ...
        <div className="lg:col-span-3 bg-white p-7 rounded-2xl border border-[#E3E1DB]">
          <div className="mb-6">
            <h4 className="text-lg font-display font-medium text-[#171A21]">Performance analysis</h4>
            <p className="text-[#6E6A62] text-sm">Strengths vs areas for improvement</p>
          </div>
          ...
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={charts.strengthsWeaknesses} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#E3E1DB" vertical={false} />
              <XAxis dataKey="name" stroke="#9C988E" fontSize={12} tickLine={false} axisLine={false} />
              <YAxis stroke="#9C988E" fontSize={12} tickLine={false} axisLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
              <Bar dataKey="value" fill="#1E5F53" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl border border-[#E3E1DB]">
          <h4 className="text-base font-display font-medium text-[#171A21] mb-5">
            Performance insights
          </h4>
          <div className="space-y-3">
            {insights.map((i, idx) => (
              <div key={idx} className="p-3.5 bg-[#DCEAE6]/50 rounded-xl border border-[#DCEAE6] text-sm">
                <p className="font-medium text-[#1E5F53]">{i.title}</p>
                <p className="text-[#171A21] mt-0.5">{i.message}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-[#E3E1DB]">
          ...
          <div className="space-y-3">
            {recommendations.length ? recommendations.map((r, idx) => (
              <div key={idx} className="p-3.5 bg-[#F3E6D8]/60 rounded-xl border border-[#F3E6D8] text-sm">
                <p className="font-medium text-[#B8763F]">{r.title}</p>
                <p className="text-[#171A21] mt-0.5">{r.message}</p>
              </div>
            )) : (
              <p className="text-[#6E6A62] text-sm">No specific recommendations at this time.</p>
            )}
          </div>
        </div>
      </div>
