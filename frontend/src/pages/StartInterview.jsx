...
//       <main className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl shadow-2xl flex w-full max-w-5xl overflow-hidden text-white">
//         {/* Sidebar */}
//         <section className="bg-white/5 w-80 p-6 rounded-l-2xl flex flex-col gap-6 border-r border-white/10">
...
//                 className="w-full mt-2 rounded-xl bg-[#1A1A1A] text-white border border-[#2D3748] p-3 focus:outline-none focus:ring-2 focus:ring-[#FF5DA2]"
//               />
//             </div>
...
//                 type="submit"
//                 disabled={loading}
//                 className={`bg-[#FF5DA2] text-white font-semibold px-6 py-2 rounded-xl transition-all ${
//                   loading ? "opacity-50 cursor-not-allowed" : "hover:bg-[#FEE440] hover:text-black"
//                 }`}
//               >
...
const steps = [
  { label: "Add your background", desc: "Job description and resume" },
  { label: "Set the shape", desc: "Difficulty, length, focus areas" },
  { label: "Begin", desc: "Answer on camera, get feedback" },
];
...
function StartInterview() {
  ...
  const selectClass =
    "w-full mt-1.5 rounded-lg bg-[#F5F6F4] text-[#171A21] border border-[#E3E1DB] px-3 py-2.5 text-sm focus:outline-none focus:border-[#1E5F53] focus:bg-white transition-colors";
  const labelClass = "text-xs font-medium tracking-wide uppercase text-[#6E6A62]";

  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F5F6F4] p-6 font-sans">
      <main className="rounded-2xl bg-white border border-[#E3E1DB] shadow-[0_1px_2px_rgba(23,26,33,0.04)] flex w-full max-w-5xl overflow-hidden">
        {/* Sidebar */}
        <section className="hidden md:flex flex-col justify-between bg-[#171A21] w-72 p-8 text-white">
          <div>
            <WaveMark />
            <h1 className="mt-5 text-2xl font-display font-medium">
              Prep<span className="italic">Pal</span>
            </h1>
            <p className="text-[#9C988E] text-sm mt-1.5">Your interview, rehearsed.</p>

            <div className="mt-12 space-y-6">
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-3">
                  <span className="font-mono text-xs text-[#9C988E] pt-0.5 w-4 shrink-0">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <div>
                    <h3 className="text-sm font-medium text-white">{step.label}</h3>
                    <p className="text-[#9C988E] text-xs mt-0.5">{step.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Form */}
        <section className="flex-1 p-8 overflow-y-auto max-h-[90vh]">
          <h2 className="text-xl font-display font-medium text-[#171A21] mb-1">Set up your session</h2>
          <p className="text-sm text-[#6E6A62] mb-6">Tell PrepPal what you're preparing for.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className={labelClass}>Résumé (PDF)</label>
              <div className="mt-1.5">
                <input
                  type="file"
                  accept=".pdf"
                  onChange={handleFileUpload}
                  className="block w-full text-sm text-[#6E6A62] file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border file:border-[#E3E1DB] file:bg-[#F5F6F4] file:text-[#171A21] file:text-sm hover:file:border-[#1E5F53] file:cursor-pointer cursor-pointer"
                />
              </div>
              {fileUpload && (
                <p className="mt-1.5 text-xs text-[#1E5F53] font-mono">{fileUpload.name}</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className={labelClass}>Interview level</label>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {["Behavioral", "Technical", "System Design", "Coding", "HR"].map((item) => (
                  <label
                    key={item}
                    className={`cursor-pointer px-3.5 py-1.5 rounded-full border text-sm transition-colors ${
                      focus.includes(item)
                        ? "bg-[#1E5F53] text-white border-[#1E5F53]"
                        : "bg-white text-[#171A21] border-[#E3E1DB] hover:border-[#1E5F53]"
                    }`}
                  >
                    <input
                      type="checkbox"
            ...
            <div className="pt-2 text-right">
              <button
                ...
                className={`px-6 py-3 rounded-lg text-sm font-medium text-white transition-colors ${
                  loading ? "bg-[#171A21]/50 cursor-not-allowed" : "bg-[#171A21] hover:bg-[#1E5F53]"
                }`}
              >
                {loading ? "Starting…" : "Start interview"}
              </button>
